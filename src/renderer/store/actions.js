...
import {SET_IS_SEARCH} from "./mutation_types";
import {SET_SEARCHED_TEXT} from "./mutation_types";
...
export default {
    ...
    [types.SEARCH_TEXT]({ commit }, {
        payload
    }) {
        commit(SET_IS_SEARCH, { payload: true })
        axios.post(`${server_address}/search`, payload)
            .then(result => {
                commit(SET_SEARCHED_TEXT, { payload: result.data })
                commit(SET_IS_SEARCH, { payload: false })
            }).catch(error => console.error(error))
    }
}
